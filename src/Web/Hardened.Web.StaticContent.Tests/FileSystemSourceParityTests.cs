...
public class FileSystemSourceParityTests : IDisposable
{
    ...
    public FileSystemSourceParityTests()
    {
        _tempRoot = Path.Combine(
            Path.GetTempPath(),
            "hardened-parity-" + Guid.NewGuid().ToString("N")
        );
    ...
    public void ALinkOutOfTheRootIsNotServed()
    {
        ...
        try
        {
            File.CreateSymbolicLink(
                Path.Combine(_staticRoot, "link.txt"),
                Path.Combine(_tempRoot, "secret.txt")
            );
        }
    ...
    public void ALinkInsideTheRootIsServed()
    {
        ...
        try
        {
            File.CreateSymbolicLink(
                Path.Combine(_staticRoot, "link.txt"),
                Path.Combine(_staticRoot, "real.txt")
            );
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
    ...
    public async Task ConcurrentFirstRequestsReadTheFileOnce()
    {
        ...
        etag.GenerateETag(Arg.Any<byte[]>())
            .Returns(_ =>
            {
        ...
        await Task.WhenAll(
            Enumerable.Range(0, 64).Select(_ => Task.Run(async () => await source.Load(location)))
        );
