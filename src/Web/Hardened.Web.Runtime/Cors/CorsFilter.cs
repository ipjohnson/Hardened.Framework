...
/// </para>
/// <para>
/// <b>Once routes declare <see cref="CorsAttribute"/>, only preflights are answered here.</b> The
/// route's own filter marks an actual request, and a preflight is answered with the policy the
/// route it asks about declares. A route that declares none refuses it.
/// </para>
/// </remarks>
public class CorsFilter : IExecutionFilter
{
    ...
    private readonly RouteCorsPolicies? _routes;
    ...
    public CorsFilter(
        ...
        : this(config, routing, routes: null) { }

    /// <param name="routes">
    /// Where a route's own declaration is read, once the application declares
    /// <see cref="CorsAttribute"/> anywhere. Null covers the whole application with
    /// <paramref name="config"/>.
    /// </param>
    internal CorsFilter(
        CorsConfiguration config,
        IEnumerable<IWebExecutionRequestHandlerProvider>? routing,
        RouteCorsPolicies? routes
    )
    {
        _config = config;
    ...
    public Task Execute(IExecutionChain chain)
    {
        ...
        if (_routes != null && !IsPreflight(request))
        {
            return chain.Next();
        }
        ...
        if (!IsPreflight(request))
        {
            ...
            {
                CorsHeaders.WriteActual(context.Response.Headers, _config, origin);
            }
        ...
        return Preflight(context, origin);
    }
    ...
    private Task Preflight(IExecutionContext context, string origin)
    {
        ...
        var allowedMethods = AllowedMethods(context, requestedMethod, out var handler);
        ...
        var policy = _routes == null ? _config : _routes.For(handler);

        if (policy == null || !policy.IsOriginAllowed(origin))
        {
            return Task.CompletedTask;
        }
        ...
        headers[KnownHeaders.Cors.AccessControlAllowOrigin] = CorsHeaders.AllowOrigin(
            policy,
            origin
        );
        headers[KnownHeaders.Cors.AccessControlAllowMethods] = allowedMethods;
        headers[KnownHeaders.Cors.AccessControlMaxAge] = policy.MaxAgeSec.ToString();
    ...
    /// like. <paramref name="handler"/> is the route that would answer, which is null on that path.
    /// </remarks>
    private string? AllowedMethods(
        IExecutionContext context,
        string requestedMethod,
        out IExecutionRequestHandler? handler
    )
    {
        handler = null;
