...
public class HostResolutionTests
{
    ...
    {
        Assert.IsType<PipelineHost>(
            Resolve(new SomeTestingAttribute(), new OtherRuntimeAttribute())
        );
    }
    ...
    public void TheNarrowestDeclarationWinsAcrossBothKinds()
    {
        Assert.IsType<PipelineHost>(
            Resolve(
                new SomeTestingAttribute(),
                new SomeRuntimeAttribute(),
                new PipelineHostAttribute()
            )
        );
        Assert.IsType<SomeHost>(
            Resolve(
                new SomeTestingAttribute(),
                new PipelineHostAttribute(),
                new SomeRuntimeAttribute()
    ...
    {
        Assert.IsType<PipelineHost>(
            Resolve(new SomeTestingAttribute(), new PipelineHostAttribute())
        );
    }
}
