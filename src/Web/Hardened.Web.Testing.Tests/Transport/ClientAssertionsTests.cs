...
public sealed record Pair<TA, TB>(TA A, TB B) : IResponseExpectation<Pair<TA, TB>>
{
    ...
    public static Pair<TA, TB> FromResponse(
        object? body,
...
public sealed class ReadingRoute : ITestClientRoute, ITestClientReader
{
    ...
    public Task<ClientAnswer?> Read(object? result, Exception? thrown, Type? bodyType)
    {
        ...
        return Task.FromResult<ClientAnswer?>(
            thrown switch
            {
                Refusal refusal => new ClientAnswer(
                    refusal.Status,
                    refusal.Body,
                    Headers(),
                    refusal.Caveat
...
public class ClientAssertionsTests
{
    ...
    public async Task AReturnedAnswerIsReadAsTheExpectation()
    {
        var reply = new Reply(
            201,
            "body",
            new Dictionary<string, string> { ["Location"] = "/todos/1" }
        );
    ...
    {
        var missing = await Task.FromException<string>(new Refusal(404, "nope"))
    ...
    public async Task AResultNoRouteRecognisesFailsNamingWhatEachRouteReads()
    {
        ...
        Assert.Contains(
            "Returns<Ok<String>>() has no route that read this call, which returned a String.",
            failure.Message
        );
    ...
    {
        var failure = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            Nothing().ReturnsStatus<NoContent>()
        );
    ...
    public async Task TheBodyTypeIsTheExpectationsTypeArgument()
    {
        ...
        await Task.FromResult(new Reply(418, "tea", Headers()))
            .Returns<Status<Http.ImATeapot, string>>();
    ...
    public async Task OutsideARunningTestItSaysSo()
    {
        ...
        using (ExecutionContext.SuppressFlow())
        {
            probe = Task.Run(
                async () =>
                    await Record.ExceptionAsync(() => Task.FromResult("x").Returns<Ok<string>>()),
