...
{
    public AnotherRoutedClient(HttpClient http, string marker)
        : base(http, marker) { }
}
...
{
    public ClaimedClient(HttpClient http, string marker)
        : base(http, marker) { }
}
...
public class TestClientBuilderTests
{
    ...
    public void ASingleHttpClientConstructorIsARoute()
    {
        ...
        var client = (ConventionClient)
            TestClientBuilder.Build(typeof(ConventionClient), context, TestAssembly);
    ...
    public void APublicFactoryInTheTestAssemblyIsARoute()
    {
        ...
        var client = (AdaptedClient)
            TestClientBuilder.Build(typeof(AdaptedClient), context, TestAssembly);
    ...
    public void ARouteTheAssemblyNamedIsARoute()
    {
        ...
        var client = (AnotherRoutedClient)
            TestClientBuilder.Build(typeof(AnotherRoutedClient), context, TestAssembly);
    ...
    public void AFactoryWinsOverARouteThatWouldAlsoBuildIt()
    {
        ...
        var client = (ClaimedClient)
            TestClientBuilder.Build(typeof(ClaimedClient), context, TestAssembly);
    ...
    public void ADeclaredTypeThatIsNotARouteFailsAtDiscovery()
    {
        var failure = Assert.Throws<InvalidOperationException>(() =>
            TestClientBuilder.DiscoverRoutes(
                [new TestClientRouteAttribute(typeof(ConventionClient))],
                "Some.Tests"
            )
        );
    ...
    public void ARouteWithNoParameterlessConstructorFailsAtDiscovery()
    {
        var failure = Assert.Throws<InvalidOperationException>(() =>
            TestClientBuilder.DiscoverRoutes(
                [new TestClientRouteAttribute(typeof(ConfiguredRoute))],
                "Some.Tests"
            )
        );
    ...
    public void TheContainerCanBuildOnlyWhatItsRegistrationsSatisfy(Type type, bool constructible)
    {
        ...
        Assert.Equal(
            constructible,
            TestClientBuilder.IsConstructibleByTheContainer(type, services)
        );
    }
    ...
    public void ARegisteredConstructorParameterMakesATypeConstructible()
    {
        ...
        Assert.True(
            TestClientBuilder.IsConstructibleByTheContainer(typeof(ConventionClient), services)
        );
    }
    ...
    public void WhatEveryProviderAnswersForCountsAsRegistered()
    {
        ...
        Assert.True(
            TestClientBuilder.IsConstructibleByTheContainer(typeof(ProviderClient), services)
        );
        Assert.True(
            TestClientBuilder.IsConstructibleByTheContainer(typeof(LoggingClient), services)
        );
        Assert.True(
            TestClientBuilder.IsConstructibleByTheContainer(typeof(DefaultedClient), services)
        );
    }
    ...
    private sealed class Case : ITestMethodContext
    {
        ...
        [Grants("method:grant")]
        public void Clients(
            ConventionClient convention,
            AdaptedClient adapted,
            OrphanClient orphan,
            HttpClient http,
            [Grants("own")] ConventionClient own
        ) { }

        [Grants("method:grant")]
        public void Attributed(
            [Grants("own")] HttpClient http,
            [Grants("own")] OrphanClient orphan,
            IServiceProvider provider,
            ProviderClient plain
        ) { }
    }
    ...
    public void AConventionClientParameterIsRegisteredOverThePipeline()
    {
        ...
        Assert.Equal(
            "method:grant",
            client.Http.DefaultRequestHeaders.GetValues("X-Test-Grants").Single()
        );
    }
    ...
    public void AnHttpClientParameterIsTheHarnessesOwn()
    {
        ...
        Assert.Equal(
            "http://harness/",
            provider.GetRequiredService<HttpClient>().BaseAddress!.ToString()
        );
    }
    ...
    public void AParameterWithNeitherRouteFailsNamingBoth()
    {
        ...
        var failure = Assert.Throws<InvalidOperationException>(() =>
            provider.GetRequiredService<OrphanClient>()
        );
    ...
    public async Task AnAttributedHttpClientIsBuiltAndANoRouteTypeStandsAside()
    {
        ...
        var httpAttribute = parameters[0]
            .GetCustomAttributes()
            .OfType<ITestParameterValueProvider>()
            .Single();
        var orphanAttribute = parameters[1]
            .GetCustomAttributes()
            .OfType<ITestParameterValueProvider>()
            .Single();

        var http = (HttpClient)
            (await httpAttribute.GetParameterValueAsync(method, provider, parameters[0]))!;
    ...
    public void TheProviderAndAConstructibleTypeAreLeftToTheContainer()
    {
        ...
        Assert.DoesNotContain(
            services,
            descriptor => descriptor.ServiceType == typeof(IServiceProvider)
        );
        Assert.DoesNotContain(
            services,
            descriptor => descriptor.ServiceType == typeof(ProviderClient)
        );
    }
    ...
    public async Task AParameterWithItsOwnCredentialIsBuiltWithIt()
    {
        ...
        var own = (ConventionClient)
            (await attribute.GetParameterValueAsync(method, provider, parameter))!;
        ...
        Assert.Equal(
            "method:grant",
            shared.Http.DefaultRequestHeaders.GetValues("X-Test-Grants").Single()
        );
    }
}
