...
public class AspNetCoreTestHostTests
{
    ...
    public async Task TheEnvironmentNameIsTheHardenedOneInScope()
    {
        ...
        await using var named = await HostHarness.Start(
            chain => chain.Next(),
            Token,
            null,
            new EnvironmentNameAttribute("development")
        );

        Assert.Equal(
            "test",
            Assert.IsType<AspNetCoreTestHost>(plain.Host).Application.Environment.EnvironmentName
        );
        Assert.Equal(
            "development",
            Assert.IsType<AspNetCoreTestHost>(named.Host).Application.Environment.EnvironmentName
        );
    }
    ...
    {
        Assert.Equal(
            typeof(AspNetCoreRuntimeAttribute),
            new AspNetCoreTestingAttribute().RuntimeAttribute
        );
    }
    ...
    {
        var failure = Assert.Throws<InvalidOperationException>(() =>
            new AspNetCoreTestingAttribute(typeof(string)).CreateHost(
                null!,
