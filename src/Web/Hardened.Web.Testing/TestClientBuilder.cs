...
internal static class TestClientBuilder
{
    ...
    > Factories = new();
    private static readonly ConcurrentDictionary<Assembly, IReadOnlyList<ITestClientRoute>> Routes =
        new();
    private static readonly ConcurrentDictionary<
        Assembly,
    ...
    public static object Build(Type clientType, TestClientContext context, Assembly testAssembly)
    {
        ...
        if (factoryType != null)
        {
            ...
            var create = typeof(ITestClientFactory<>)
    ...
    public static IReadOnlyList<ITestClientReader> ReadersFor(Assembly testAssembly) =>
        Readers.GetOrAdd(
            testAssembly,
            assembly => RoutesFor(assembly).OfType<ITestClientReader>().ToArray()
        );
    ...
    private static IReadOnlyList<ITestClientRoute> DiscoverRoutes(Assembly assembly) =>
        DiscoverRoutes(
            assembly.GetCustomAttributes<TestClientRouteAttribute>(),
            assembly.GetName().Name!
        );

    internal static IReadOnlyList<ITestClientRoute> DiscoverRoutes(
        IEnumerable<TestClientRouteAttribute> attributes,
        string assemblyName
    )
    {
    ...
    private static Type? FactoryFor(Type clientType, Assembly testAssembly) =>
        Factories.GetOrAdd(testAssembly, Discover).TryGetValue(clientType, out var factory)
            ? factory
            : null;
        ...
        foreach (var type in assembly.GetTypes())
        {
            ...
            foreach (var contract in type.GetInterfaces())
            {
                if (
                    ...
                    && contract.GetGenericTypeDefinition() == typeof(ITestClientFactory<>)
                )
                {
