...
public class ResolvedPrincipalTests
{
    ...
    private static async Task<ICallerPrincipal> Resolved(
        ICallerPrincipal caller,
        string[] required,
        params string[] resolvable
    )
    {
        ...
        await Pipeline
            .Chain(context, new AuthorizationFilter(requirement, beforeSerialization: true))
            .Next();
    ...
    public async Task EnumeratingTheUnionYieldsEachGrantOnce()
    {
        ...
        Assert.Equal(
            ["pets:read", "pets:write"],
            resolved.Grants.OrderBy(grant => grant, StringComparer.Ordinal)
        );
    }
    ...
    public async Task TheWrapperIsNotInstalledOnTheContext()
    {
        ...
        var context = Pipeline.Context(configureServices: services =>
        {
            ...
            services.AddSingleton<IActivityAuthorizationHandler>(
                new ResolvingHandler("pets:write")
            );
        });
