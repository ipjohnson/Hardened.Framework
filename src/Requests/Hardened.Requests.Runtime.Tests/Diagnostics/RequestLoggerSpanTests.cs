...
public class RequestLoggerSpanTests
{
    ...
    public void AFailureCarriesTheStackOnAnExceptionEvent()
    {
        ...
        Assert.Equal(
            ...
            recorded.Tags.Single(t => t.Key == "exception.type").Value
        );
        Assert.Equal(
            "handler blew up",
            recorded.Tags.Single(t => t.Key == "exception.message").Value
        );
        Assert.Contains(
            "InvalidOperationException",
            (string)recorded.Tags.Single(t => t.Key == "exception.stacktrace").Value!
        );
    }
    ...
    private static Dictionary<string, StringValues> Headers(
