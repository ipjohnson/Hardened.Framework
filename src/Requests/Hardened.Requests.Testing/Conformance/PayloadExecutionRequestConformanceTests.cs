...
/// <summary>
/// The behaviour every <see cref="IExecutionRequest"/> implementation must exhibit, whatever
/// shape its request is, expressed once and executed against every transport.
///
/// Hardened's central claim is that it is transport agnostic: a handler should not be able
/// to tell whether its request arrived over ASP.NET Core, API Gateway, a queue or an
/// in-memory test harness. That claim is only true if every adapter maps its native
/// request onto this contract the same way. This suite is what makes that checkable rather
/// than assumed.
///
/// <para>
/// <b>This is the payload-shaped profile, and the smaller of the two.</b> Per D7 there are two
/// request shapes: web-shaped carries a path, query, headers, cookies and a body; payload-shaped
/// carries a record. A queue message has no query string and no cookies - not an empty one, no
/// such concept - so an adapter for it cannot surface either however it is written, and
/// answering empty is the honest answer
/// rather than a gap. Asserting web semantics against it would be asserting
/// something untrue about the transport.
/// </para>
///
/// <para>
/// Everything here holds for both shapes: the method, the path, headers and the content type and
/// accept read out of them, the body, path tokens, transport info, and every <c>Clone</c>
/// behaviour. That is twenty-three of the twenty-six assertions this suite used to carry in one
/// class; the other three are in <see cref="ExecutionRequestConformanceTests"/>, which derives
/// from this and is what a web transport enrols in.
/// </para>
///
/// To enrol a payload-shaped transport, derive from this class and supply an adapter:
///
/// <code>
/// public class MyQueueConformanceTests : PayloadExecutionRequestConformanceTests {
...
public abstract class PayloadExecutionRequestConformanceTests {
    ...
    protected string Because(string what) => $"[{Adapter.TransportName}] {what}";
