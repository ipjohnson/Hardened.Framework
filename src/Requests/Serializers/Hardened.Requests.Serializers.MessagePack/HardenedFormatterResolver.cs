...
public sealed class HardenedFormatterResolver : IFormatterResolver
{
    ...
    private static object? Lookup(Type type)
    {
        ...
        if (type == typeof(RequestValidationFieldError))
            return RequestValidationFieldErrorFormatter.Instance;

        // The four response types that carry a member beyond detail, and the one thing one of them
        // carries.
        if (type == typeof(NotFound))
            return NotFoundFormatter.Instance;
        if (type == typeof(RateLimited))
            return RateLimitedFormatter.Instance;
        if (type == typeof(ServiceUnavailable))
            return ServiceUnavailableFormatter.Instance;
        if (type == typeof(Unauthorized))
            return UnauthorizedFormatter.Instance;
        if (type == typeof(AuthorizationChallenge))
            return AuthorizationChallengeFormatter.Instance;
        ...
        if (type == typeof(PaymentRequired))
            return Problem<PaymentRequired>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new PaymentRequired(d)
            );
        if (type == typeof(Forbidden))
        ...
        if (type == typeof(Gone))
            return Problem<Gone>(v => v.Type, v => v.Title, v => v.Detail, d => new Gone(d));
        if (type == typeof(PreconditionFailed))
            return Problem<PreconditionFailed>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new PreconditionFailed(d)
            );
        if (type == typeof(PreconditionRequired))
            return Problem<PreconditionRequired>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new PreconditionRequired(d)
            );
        if (type == typeof(ContentTooLarge))
            return Problem<ContentTooLarge>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new ContentTooLarge(d)
            );
        if (type == typeof(UnsupportedMediaType))
            return Problem<UnsupportedMediaType>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new UnsupportedMediaType(d)
            );
        if (type == typeof(UnprocessableContent))
            return Problem<UnprocessableContent>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new UnprocessableContent(d)
            );
        if (type == typeof(RequestTimeout))
            return Problem<RequestTimeout>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new RequestTimeout(d)
            );
        if (type == typeof(InternalServerError))
            return Problem<InternalServerError>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new InternalServerError(d)
            );
        if (type == typeof(NotImplemented))
            return Problem<NotImplemented>(
                ...
                v => v.Title,
                v => v.Detail,
                d => new NotImplemented(d)
            );
        if (type == typeof(BadGateway))
            return Problem<BadGateway>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new BadGateway(d)
        ...
        return type == typeof(GatewayTimeout)
            ? Problem<GatewayTimeout>(
                v => v.Type,
                v => v.Title,
                v => v.Detail,
                d => new GatewayTimeout(d)
            )
            : null;
    }
}
