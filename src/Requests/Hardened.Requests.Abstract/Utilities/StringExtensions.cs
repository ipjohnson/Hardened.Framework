...
public static class StringExtensions
{
    ...
    private static StringValues SplitString(ReadOnlySpan<char> span, int commaCount)
    {
        ...
        for (var i = 0; i < span.Length; )
        {
            if (span[i] == ',')
            {
                stringArray[stringCount] = span.Slice(currentStringIndex, i - currentStringIndex)
                    .ToString();

                stringCount++;
                currentStringIndex = i + 1;

                if (currentStringIndex < span.Length - 1 && span[currentStringIndex] == ' ')
                {
                    currentStringIndex++;
                    i++;
                }
        ...
        {
            stringArray[stringCount] = span.Slice(
                    currentStringIndex,
                    span.Length - currentStringIndex
                )
                .ToString();
        }
        else
