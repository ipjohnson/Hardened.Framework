...
internal class AsyncInvokeNoParametersFilter<TController> : IExecutionFilter
    where TController : class
{
    private readonly ExecutionHelper.AsyncInvokeNoParameters<TController> _invoke;

    public AsyncInvokeNoParametersFilter(
        ExecutionHelper.AsyncInvokeNoParameters<TController> invoke
    )
    {
        _invoke = invoke;
    }
    ...
    public async Task Execute(IExecutionChain chain)
    {
        ...
        try
        {
            ...
            context.RequestMetrics.Record(
                RequestMetrics.HandlerInvokeDuration,
