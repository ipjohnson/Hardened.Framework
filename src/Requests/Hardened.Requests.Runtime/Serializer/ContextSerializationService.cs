...
public class ContextSerializationService : IContextSerializationService
{
    ...
    )
    {
        ...
        // Whatever the client asked for. A handler that declares an output has said what its
        // response is, and the Accept header does not get a vote - see IHardenedResponseOutput.
        if (output != null)
        {
            return output.WriteOutput(context);
        }
