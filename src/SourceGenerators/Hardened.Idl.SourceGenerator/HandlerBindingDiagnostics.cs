...
internal static class HandlerBindingDiagnostics
{
    ...
    /// </para>
    /// </remarks>
    private static readonly Dictionary<string, string> InertDeclarations = new(
    ...
    )
    {
        ...
        ["TagAttribute"] = "a described operation is grouped by the tag its contract gives it",
        ["ServerAttribute"] =
    ...
    /// </summary>
    private static DiagnosticDescriptor NoHandlerDescriptor() =>
        new(
            id: NoHandlerId,
            title: "Described service has no handler",
            messageFormat: "'{0}' is declared by the description but no class carrying [Handler] implements it, so "
                + "its {1} route(s) exist and fail at request time. Implement it, or set "
                + "<NoWarn>$(NoWarn);"
                + NoHandlerId
                + "</NoWarn> if this project ships the interface alone.",
            category: "Hardened.Generation",
            defaultSeverity: DiagnosticSeverity.Warning,
            isEnabledByDefault: true
        );

    private static DiagnosticDescriptor InertDeclarationDescriptor() =>
        new(
            id: InertDeclarationId,
            title: "Declaration is not read on a described handler",
            messageFormat: "'{0}' carries [{1}], which is read from a handler's own syntax and a described "
    ...
    private static DiagnosticDescriptor NoServiceInterfaceDescriptor() =>
        new(
            id: NoServiceInterfaceId,
            title: "Handler implements no described service",
            messageFormat: "'{0}' carries [Handler] but its base list names no service the description declares - "
    ...
    )
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            diagnostics.Add(
                Diagnostic.Create(
                    ...
                    string.Join(
                        ", ",
                        handler.InterfaceCandidates.Select(candidate => candidate.Name)
                    ),
        ...
        foreach (var name in declaredNames.OrderBy(name => name, StringComparer.Ordinal))
        {
            ...
            diagnostics.Add(
                Diagnostic.Create(NoHandlerDescriptor(), Location.None, name, routeCount)
            );
        }
        ...
        foreach (var method in handler.MethodFilters)
        {
            foreach (var filter in method.Filters)
            {
                Report(
                    handler,
                    filter,
                    handler.ImplementationType.Name + "." + method.MethodName,
                    diagnostics
                );
            }
        }
