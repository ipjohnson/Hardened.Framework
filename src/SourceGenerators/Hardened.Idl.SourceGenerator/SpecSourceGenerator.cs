...
public class SpecSourceGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var parseResults = context
            .AdditionalTextsProvider.Where(IsSpecModelFile)
            .Select(ReadSpecModel);

        var openApiFiles = parseResults.Where(r => r.Model != null).Select((r, _) => r.Model)!;

        // Diagnostic: report AdditionalTexts state and any parse errors
        var diagProvider = context
...
"
        ...
        var configProvider = context.AnalyzerConfigOptionsProvider.Select(
            (options, _) =>
            {
                ...
                options.GlobalOptions.TryGetValue(
                    "build_property.HardenedIdlNamespace",
                    out var ns
                );
                if (string.IsNullOrEmpty(ns))
                {
                    options.GlobalOptions.TryGetValue(
                        "build_property.HardenedOpenApiNamespace",
                        out ns
                    );
                }
                if (string.IsNullOrEmpty(ns))
                ...
                if (
                    options.GlobalOptions.TryGetValue(
                        "build_property.ExcludeGeneratedCodeFromCoverage",
                        out var excludeValue
                    )
                )
                {
                    excludeFromCoverage = !string.Equals(
                        excludeValue,
                        "false",
        ...
        var handlerModels = specWithNamespace.Select(
            (pair, ct) =>
            {
                if (pair.Left == null)
                    return ImmutableArray<RequestHandlerModel>.Empty;
                var ns = pair.Right;
        ...
        var handlerInfoProvider = context
        ...
        var allHandlerModels = handlerModels
            .Collect()
            .Select(
                (arrays, _) =>
                {
        ...
        var enrichedModels = allHandlerModels
            ...
            .Select(
                (pair, _) =>
                {
                    ...
                    if (handlerInfos.Length == 0)
                        return models;
                    return RequestModelBuilder
                        .EnrichWithHandlerFilters(models.ToList(), handlerInfos!)
                        .ToImmutableArray();
                }
            );
        ...
        context.RegisterSourceOutput(
            ...
            (ctx, pair) =>
                HandlerBindingDiagnostics.Report(
                    ctx,
                    pair.Left,
                    pair.Right.Where(info => info != null).Select(info => info!).ToList()
                )
        );
        ...
        var excludeFromCoverageProvider = configProvider.Select(
            (cfg, _) => cfg.ExcludeFromCoverage
        );
        ...
        var identityProvider = openApiFiles
            .Collect()
            .Select(
                (specs, _) =>
                {
                    ...
                    (
                        (
                            (
                                ...
                                ImmutableArray<HandlerInfo?> Right
                            ) Left,
                            ImmutableArray<SpecRegistration> Right
                        ) Left,
                        bool Right
                    ) Left,
                    ImmutableArray<RouteConstraintModel> Right
                ) Left,
                DocumentIdentity Right
            )>(
                (ctx, pair) =>
                    SpecRoutingTableGenerator.GenerateRoute(
                        ctx,
                        pair.Left.Left.Left.Left.Left,
    ...
    private static (ServiceSpecModel? Model, string? Error) ReadSpecModel(
        AdditionalText text,
