...
public static class RouteConstraintSelector
{
    ...
    /// </summary>
    private static DiagnosticDescriptor SignatureDescriptor() =>
        new(
            id: SignatureDiagnosticId,
            title: "Route constraint has the wrong signature",
            messageFormat: "'{0}' declares the route constraint '{1}' but is not a static bool(ReadOnlySpan<char>). "
    ...
    )
    {
        ...
        return (IReadOnlyList<RouteConstraintModel>?)constraints
            ?? Array.Empty<RouteConstraintModel>();
    }
    ...
    )
    {
        ...
        return parameterType
            .Replace(" ", "")
            .EndsWith("ReadOnlySpan<char>", StringComparison.Ordinal);
    }
