...
{
    public static HardenedMethodDefinition GetMethodDefinition(
    ...
    private static ITypeDefinition? GetReturnType(
    ...
    )
    {
        ...
        foreach (var parameter in methodDeclarationSyntax.ParameterList.Parameters)
        {
            ...
            {
                var type = parameter.Type?.GetTypeDefinition(generatorSyntaxContext)!;

                parameters.Add(
                    new HardenedParameterDefinition(parameter.Identifier.ValueText, type)
                );
            }
        }
