...
public static class ConfigurationEntryPointGenerator
{
    ...
    )
    {
        ...
        arg1.AddSource(
            arg2.AppModel.EntryPointType.Name + ".Configuration.cs",
    ...
    )
    {
        ...
        GenerateDependencyInjectionRegistration(
            classDefinition,
            entryPoint,
            configFiles,
            providerType
    ...
    )
    {
        ...
        {
            var ioptionsType = new GenericTypeDefinition(
                TypeDefinitionEnum.ClassDefinition,
                "Microsoft.Extensions.Options",
                "IOptions",
                new[] { configurationFileModel.InterfaceType }
            );
    ...
    )
    {
        ...
        foreach (var configurationFileModel in configFiles)
        {
            var initConfigValue = CreateInitConfigValueMethod(
                classDefinition,
    ...
    private static string CreateInitConfigValueMethod(
        ClassDefinition classDefinition,
        ConfigurationIncrementalGenerator.ConfigurationFileModel configurationFileModel
    )
    {
        var modelArray = configurationFileModel
            .FieldModels.Where(m => !string.IsNullOrEmpty(m.FromEnvironmentVariable))
            .ToArray();
        ...
        if (modelArray.Length > 0)
        {
            ...
            foreach (var configurationFieldModel in modelArray)
            {
                ...
                var invoke = env.Invoke(
                    "Value",
                    configurationFieldModel.FromEnvironmentVariable,
                    propertyAccess
                );
