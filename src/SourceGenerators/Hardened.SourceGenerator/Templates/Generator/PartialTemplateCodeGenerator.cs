...
public class PartialTemplateCodeGenerator {
    ...
    public void ProcessPartialTemplateNode(TemplateImplementationGenerator.GenerationContext context) {
        if (context.CurrentNode!.ArgumentList.Count is > 0 and <= 2) {
            ...
            context.CurrentBlock.AddCode(
                "await [arg1]([arg2], serviceProvider, writer, executionContext, requestExecutionContext);",
                partialTemplateFunction, argument);
        }
    }
    ...
    private string GetOrCreatePartialTemplateFunctionField(TemplateImplementationGenerator.GenerationContext context,
        string actionText) {
