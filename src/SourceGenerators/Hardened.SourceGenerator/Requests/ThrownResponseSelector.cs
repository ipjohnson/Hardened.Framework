...
public static class ThrownResponseSelector
{
    ...
    private const string ValidationErrorType =
        "Hardened.Requests.Runtime.Validation.RequestValidationError";
    ...
    private static DiagnosticDescriptor Descriptor() =>
        new(
            ...
            messageFormat: "'[Throws<{0}>]' on '{1}' names a type carrying no [HttpStatus], and states no status of "
    ...
    public static void Report(SourceProductionContext context, string handler, string? unresolved)
    {
    ...
    )
    {
        ...
        return (IReadOnlyList<ResponseSchemaModel>?)declared
    ...
    private static INamedTypeSymbol? ThrownType(
        GeneratorSyntaxContext context,
    ...
    {
        var named = attribute.ArgumentList?.Arguments.FirstOrDefault(candidate =>
            candidate.NameEquals?.Name.Identifier.Text == "Description"
        );

        return
            named != null
            && context.SemanticModel.GetConstantValue(named.Expression)
