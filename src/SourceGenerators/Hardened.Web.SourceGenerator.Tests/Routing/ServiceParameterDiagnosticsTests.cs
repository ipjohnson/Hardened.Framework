...
public class ServiceParameterDiagnosticsTests
{
    ...
    private static Diagnostic? Reported(string types, string parameters) =>
        Generate(types, parameters)
            .GeneratorDiagnostics.SingleOrDefault(reported => reported.Id == DiagnosticId);
    ...
    // A plain body model.
    [InlineData(
        "public class EventBody { public string Title { get; set; } = \"\"; }",
        "EventBody body"
    )]
    // An immutable body model, whose constructor takes its own data rather than a service.
    [InlineData(
        "public class EventBody { public EventBody(string title) { Title = title; } "
    ...
    [InlineData(
        "public class EventBody { public EventBody() { } public EventBody(IClock c) { } }",
        "EventBody body"
    )]
    // The correct spelling of the reported case: still a service, no longer the body.
    [InlineData(
        "public class EventStore { public EventStore(IClock clock) { } }",
