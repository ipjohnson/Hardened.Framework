...
public class CompressAttributeGenerationTests
{
    ...
    private static IEnumerable<Diagnostic> Reported(
        string classAttributes,
        string methodAttributes
    ) =>
        Generate(classAttributes, methodAttributes)
            .GeneratorDiagnostics.Where(reported => reported.Id == DiagnosticId);
    ...
    public void ThePredicateFormWithAnIntegerAndAFavourCompiles()
    {
        ...
        Assert.Contains(
            "global::Hardened.Web.Runtime.Compression.CompressAttribute<global::TestApp.ListLargerThan>(50)",
            source
        );
        Assert.Contains(
            "Favor = global::Hardened.Web.Runtime.Compression.CompressionType.Br",
            source
        );
    }
    ...
    public void ThePlainFormOnAMethodCompiles()
    {
        ...
        Assert.Contains(
            "Favor = global::Hardened.Web.Runtime.Compression.CompressionType.GZip",
            source
        );
    }
    ...
    public void ThePlainFormOnAClassReachesEveryHandlersMetadata()
    {
        ...
        Assert.Contains(
            "global::Hardened.Web.Runtime.Compression.CompressAttribute()",
            result.SourceContaining("PetsController_List")
        );
    }
    ...
    [InlineData("", "[Compress] [Compress<ListLargerThan>(2)]")]
    public void ADeclarationOnBothTheClassAndTheMethodIsAnError(
        string classAttributes,
        string methodAttributes
    )
    {
