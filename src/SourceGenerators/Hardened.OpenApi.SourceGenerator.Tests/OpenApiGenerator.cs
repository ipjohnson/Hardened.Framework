...
internal static class OpenApiGenerator {
    ...
    /// <summary>
    /// The single file <c>ExtractOpenApiSpec</c> writes into <c>@(Compile)</c> for one spec,
    /// composed from the same emitters the task calls rather than from a copy of their output.
    /// </summary>
    internal static KeyValuePair<string, string> TaskEmittedSource(
        ...
        foreach (var schema in model.Schemas) {
            ...
            if (emitted is not null) {
                builder.AppendLine(emitted);
            }
        }

        foreach (var service in model.Services) {
            builder.AppendLine(ServiceInterfaceEmitter.Emit(service, ns));
        }

        builder.AppendLine(JsonTypeInfoEmitter.Emit(model.Schemas, ns, model.FileName, excludeFromCoverage));
        ...
        return new KeyValuePair<string, string>($"{model.FileName}.g.cs", builder.ToString());
    }
