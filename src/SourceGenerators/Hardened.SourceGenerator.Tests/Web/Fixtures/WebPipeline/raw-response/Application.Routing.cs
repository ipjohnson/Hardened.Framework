...
;
        ...
        private static void RoutingTableDI(IServiceCollection serviceCollection)
        {
            ...
            >();
            serviceCollection.AddTransient<BlobController>();
        ...
        private class RoutingTable : IWebExecutionRequestHandlerProvider
        {
            ...
            private RequestHandlerInfo? _infoBlobController_Blob;
                ...
                if ((charSpan.Length >= index + 4) && (charSpan[index + 0] == 'b') && (charSpan[index + 1] == 'l') && (charSpan[index + 2] == 'o') && (charSpan[index + 3] == 'b'))
                {
                    ...
                    if (charSpan.Length == index)
                    {
                        switch (methodString)
                        {
                            case "HEAD":
                            case "GET":
                                return _infoBlobController_Blob ??= new RequestHandlerInfo(
                                    new BlobController_Blob(_rootServiceProvider),
