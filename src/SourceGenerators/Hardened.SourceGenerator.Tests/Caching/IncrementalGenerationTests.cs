...
public class IncrementalGenerationTests
{
    ...
    public void AddingANonHandlerMethodReusesEveryOutput()
    {
        var result = Rerun(
            Controller(),
            Controller(extraMembers: "private string Helper() => \"x\";")
        );
    ...
    {
        AssertRegenerated(Rerun(Controller(), Controller().Replace("[Get(", "[Post(")));
    }
    ...
    public void RenamingAPathTokenRegeneratesTheHandler()
    {
        AssertRegenerated(
            Rerun(
                ...
                Controller(
                    path: "/orders/{orderId}",
                    signature: "public string GetOrder(string orderId) => orderId;"
                )
            )
        );
    }
    ...
    {
        AssertRegenerated(
            Rerun(Controller(), Controller(signature: "public void GetOrder(string id) { }"))
        );
    }
    ...
    public void MakingAHandlerAsyncRegeneratesTheHandler()
    {
        AssertRegenerated(
            Rerun(
                Controller(),
                Controller(
                    signature: "public Task<string> GetOrder(string id) => Task.FromResult(id);"
                )
            )
        );
    }
    ...
    {
        AssertRegenerated(
            Rerun(Controller(), Controller(methodAttributes: "[Retry(Retries = 2)]"))
        );
    }
