...
internal static class ProblemConversionEmitter
{
    public static ClassDefinition? Emit(
        ...
        IReadOnlyList<ProblemConversion.Plan> plans,
    ...
    )
    {
        ...
        foreach (
            var plan in plans.OrderBy(candidate => candidate.Key, System.StringComparer.Ordinal)
        )
        {
    ...
    )
    {
        ...
        return plan.CaseIsShipped
            ? "new "
                + caseType
                + "("
                + ShippedResponses.GenericArguments(plan.StatusCode, payload, record)
                + ")"
