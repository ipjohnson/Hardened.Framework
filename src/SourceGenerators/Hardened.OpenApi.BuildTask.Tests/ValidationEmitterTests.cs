...
public class ValidationEmitterTests
{
    ...
    {
        Assert.DoesNotContain("interface", EmitInterfaces(Spec(Operation("GetPet", Parameter()))));
    }

    [Fact]
    public void EveryConstrainedOperationInTheSpecGetsOne()
    {
        var output = EmitInterfaces(
            Spec(
    ...
    public void EveryServiceInTheSpecContributes()
    {
        var spec = new ServiceSpecModel
        {
            ...
            [
                ...
                {
                    Tag = "pets",
                    Operations = [Operation("GetPet", Parameter(minLength: 1))],
                },
                new ServiceModel
                {
                    Tag = "store",
                    Operations = [Operation("GetOrder", Parameter("orderId", minLength: 1))],
                },
            ],
            Schemas = [],
        };
    ...
    {
        var output = EmitInterfaces(
            Spec(Operation("GetPet", Parameter("petId", minLength: 1), Parameter("name")))
        );
    ...
    private static string EmitPatterns(PatternRegistry patterns) =>
        EmitterHarness.Write(
            ns => ValidationEmitter.EmitPatterns(ns, patterns),
            ValidationNamespace
        );
    ...
    public void TheMemberIsABodylessPartialMethod()
    {
        ...
        Assert.Contains(
            "public static partial global::System.Text.RegularExpressions.Regex P_c37a8736();",
            output
        );
