...
public class ProblemConversionEmitterTests
{
    ...
    public void TheDefaultInstanceConvertsToACachedCase()
    {
        ...
        Assert.Contains(
            $"ReferenceEquals(value, {Shipped}NotFound.Default) ? NotFoundProblemDefault :",
            emitted
        );
        Assert.Contains(
    ...
    public void AHeaderCarryingRecordPassesItsHeaderOn()
    {
        ...
        Assert.Contains(
            $"new {Shipped}RateLimited<{Models}Problem>(value.RetryAfter, new {Models}Problem(",
            emitted
        );
        Assert.Contains(
            $"new {Shipped}ServiceUnavailable<{Models}Problem>(new {Models}Problem(",
            emitted
        );
