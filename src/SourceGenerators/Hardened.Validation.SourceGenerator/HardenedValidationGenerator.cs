...
public class HardenedValidationGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterPostInitializationOutput(static production =>
            production.AddSource(
                "Hardened.Validation.Marker.g.cs",
                GeneratedSource.Header(MarkerSource)
            )
        );
            ...
            static (production, result) =>
            {
                ...
                if (result.Model is { } model)
                {
                    ...
                    try
                    {
                        production.AddSource(
                            HintNameFor(model),
                            GeneratedSource.Header(new ValidatorEmitter().Emit(model))
                        );
                    }
                    catch (ArgumentException exception)
                    {
                        production.ReportDiagnostic(
                            ValidationGeneratorDiagnostics.DuplicateValidatorSource(
                                model,
                                exception
                            )
                        );
        ...
        context.RegisterSourceOutput(
            entryPoint.Combine(validators),
            static (production, pair) => RegistrationWriter.Write(production, pair.Left, pair.Right)
        );
    }
    ...
    )
    {
        ...
        foreach (var member in symbol.GetMembers())
        {
            ...
            if (
                nested == null
                || SymbolEqualityComparer.Default.Equals(nested, symbol)
                || !DeclaredInSource(nested)
                || !CarriesConstraints(nested)
            )
            {
                continue;
            }
    ...
    private static bool IsConstraint(INamedTypeSymbol? attributeClass)
    {
        for (var type = attributeClass; type != null; type = type.BaseType)
        {
            ...
            if (
                name
                is "ValidationModules.Constraints.ValidationConstraintAttribute"
    ...
    private static bool DeclaresRequired(IPropertySymbol property)
    {
        foreach (var attribute in property.GetAttributes())
        {
            ...
            if (
                name
                is "ValidationModules.Constraints.RequiredAttribute"
