...
public class ApplicationLogicTests
{
    ...
    private sealed class GatedStartupService(Func<IServiceProvider, Task<bool>> startup)
        : IStartupService
    {
    ...
    public async Task StartSucceedsWhenEveryStartupServiceSucceeds()
    {
        Assert.Equal(
            0,
            await ApplicationLogic.Start(Provider(Returning(true), Returning(true)), null)
        );
    }
    ...
    public async Task StartFailsWhenAnyStartupServiceReturnsFalse()
    {
        Assert.Equal(
            1,
            await ApplicationLogic.Start(Provider(Returning(true), Returning(false)), null)
        );
    }
    ...
    public async Task StartFailsWhenEveryStartupServiceReturnsFalse()
    {
        Assert.Equal(
            1,
            await ApplicationLogic.Start(Provider(Returning(false), Returning(false)), null)
        );
    }
    ...
    public async Task TheStartupTaskArgumentRunsAlongsideTheStartupServices()
    {
        ...
        var result = await ApplicationLogic.Start(
            Provider(Returning(true)),
            _ =>
            {
                taskRan = true;
                return Task.FromResult(true);
    ...
    public async Task AFailingStartupTaskArgumentFailsStartup()
    {
        Assert.Equal(
            1,
            await ApplicationLogic.Start(Provider(Returning(true)), _ => Task.FromResult(false))
        );
    }
    ...
    public async Task EachStartupServiceReceivesTheRootProvider()
    {
        ...
        var withService = Provider(
            new GatedStartupService(serviceProvider =>
            {
                received = serviceProvider;
                return Task.FromResult(true);
            })
    ...
    public async Task RunApplicationDoesNotRunTheDelegateWhenStartupFails()
    {
        ...
        var result = await ApplicationLogic.RunApplication(
