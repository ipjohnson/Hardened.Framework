...
public class ServiceProviderCompositionTests {
    ...
    private static Hardened.Amz.SourceGeneration.Testing.GeneratorResult Generate(string source) {
        var result = FunctionGeneratorHarness.Run(
            [new LambdaFunctionSourceGenerator(), new ServiceProviderGenerator()], source);
    ...
    public void TheEmittedConstructorCallsTheEmittedServiceProviderFactory() {
        ...
        FunctionGeneratorHarness.AssertEmits(result.SourceContaining("LambdaApplication"),
    ...
    public void TheRegisterInitDiSeamBindsToTheFactorysInitialisationDelegate() {
        var factory = Generate(Application()).SourceContaining("ServiceProvider.cs");

        Assert.Contains("initDependencies?.Invoke(environment, serviceCollection);", factory);
    }
