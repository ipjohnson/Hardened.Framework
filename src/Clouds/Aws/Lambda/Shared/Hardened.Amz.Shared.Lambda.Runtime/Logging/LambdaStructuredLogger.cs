...
public class LambdaStructuredLogger : ILogger {
    ...
    public LambdaStructuredLogger(IJsonSerializer jsonSerializer, ILambdaContextAccessor lambdaContextAccessor,
        string categoryName) {
    ...
    private StructuredTupleData GetRecordDataTuple<TState>(LogLevel logLevel, EventId eventId, TState state,
        IEnumerable<Tuple<string, object>> data, Exception? exception, Func<TState, Exception, string> formatter) {
...
public partial class StructuredTupleData {
    public StructuredTupleData(DateTime timestamp, string requestId, string logger, string logLevel, EventId eventId,
        string message, IEnumerable<Tuple<string, object>> data, ExceptionRecord? exception) {
        Timestamp = timestamp;
        RequestId = requestId;
        Logger = logger;
        LogLevel = logLevel;
...
public class StructuredLogEntry<TState> {
    public StructuredLogEntry(
        DateTime timestamp,
        string requestId, string logger, string logLevel, EventId eventId, string message, TState data,
