...
/// </para>
/// </summary>
public class LocalDynamoDbAttribute
    : Attribute,
        ...
        IHardenedTestStartupAttribute
{
    ...
    )
    {
        ...
        serviceCollection.AddSingleton<IDynamoDbClientProvider>(_ => new ContainerClientProvider(
            image
        ));
    }
    ...
    /// </summary>
    private sealed class ContainerClientProvider(string image)
        : IDynamoDbClientProvider,
            IDisposable
    {
        private readonly Lazy<IAmazonDynamoDB> _client = new(() =>
            LocalDynamoDb.CreateClient(image)
        );
