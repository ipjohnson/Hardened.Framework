...
public class EventSourceAdapterTests
{
    ...
    public void ASqsForkCarriesMessageAttributesAsHeaders()
    {
        ...
        Assert.Equal(
            ...
            fork.Headers[SqsRequest.MessageIdHeader]
        );
        Assert.Equal(
    ...
    public async Task SqsReportsAnEmptyFailureList()
    {
        ...
        await adapter.WriteResponse(
            new ResponseOnlyContext(adapter.CreateResponse(new MemoryStream()), batch),
            output
        );
    ...
    public async Task AFailedMessageIsNamedInTheReport()
    {
        using var payload = Payloads.Sqs(
            new SQSEvent.SQSMessage
            {
                MessageId = "m0",
                Body = "{}",
                EventSourceArn = "arn:aws:sqs:r:a:q",
            },
            new SQSEvent.SQSMessage
            {
                MessageId = "m1",
    ...
    public void PartialBatchFailuresAreOffUnlessAskedFor()
    {
        ...
        Assert.False(
            (
                (SqsRequest)new SqsAdapter().CreateRequest(payload, TestLambdaContext.Instance)
            ).ReportsItemFailures
        );
        ...
        Assert.True(
            (
                (SqsRequest)
                    new SqsAdapter(reportsItemFailures: true).CreateRequest(
                        payload,
    ...
    public void SnsRefusesToRecordAnItemFailure()
    {
        ...
        var delivery = (SnsRequest)
            new SnsAdapter().CreateRequest(payload, TestLambdaContext.Instance);
