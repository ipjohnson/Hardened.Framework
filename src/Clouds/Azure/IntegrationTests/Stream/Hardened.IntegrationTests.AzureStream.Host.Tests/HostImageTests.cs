...
public sealed class HostImageTests : IClassFixture<HostImageTests.FunctionApp>
{
    ...
    public async Task EventsPublishedToTheEmulatorReachTheHandlerInOrder()
    {
        ...
        await _app.Simulator.Observed.WaitFor(
            one => one.Has("id", "host-2"),
            cancellationToken: TestContext.Current.CancellationToken
        );

        var observed = (
            await _app.Simulator.Observed.Current(TestContext.Current.CancellationToken)
        )
        ...
        await _app.Simulator.HostLogContaining(
            "Executed 'Functions.Stream_clickstream' (Succeeded",
            cancellationToken: TestContext.Current.CancellationToken
        );
    }
    ...
    {
        public StreamHostSimulator Simulator { get; } =
            new(ApplicationOutput.Of("Hardened.IntegrationTests.AzureStream.SUT"));
        ...
        public async ValueTask InitializeAsync()
        {
            ...
            _producer = new EventHubProducerClient(
                Simulator.PublisherConnectionString,
                StreamHostSimulator.Hub
            );
        }
