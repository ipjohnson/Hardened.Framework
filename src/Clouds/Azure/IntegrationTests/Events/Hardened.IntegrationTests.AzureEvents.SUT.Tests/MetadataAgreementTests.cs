...
public class MetadataAgreementTests
{
    private static Task<ImmutableArray<IFunctionMetadata>> Declared() =>
        new AzureEventsTestAppAzureFunctionMetadataProvider().GetFunctionMetadataAsync(
    ...
    public async Task TheProviderAndTheBuildTaskAgree()
    {
        Assert.Empty(
            await MetadataAgreement.Disagreements(
                new AzureEventsTestAppAzureFunctionMetadataProvider()
            )
        );
    }
    ...
    public async Task TheBindingsCarryWhatTheApplicationSaid()
    {
        ...
        var topic = declared
            .Single(function => function.Name == "Topic_order_events")
            .RawBindings!.Single();
        var timer = declared
            .Single(function => function.Name == "Timer_nightly_rollup")
            .RawBindings!.Single();
