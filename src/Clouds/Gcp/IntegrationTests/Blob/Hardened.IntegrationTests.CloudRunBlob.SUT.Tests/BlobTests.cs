...
public class BlobTests
{
    ...
    )
    {
        ...
        sink.Received()
            .Arrived(
                Arg.Is<Upload>(upload =>
                    upload.Bucket == "uploads" && upload.EventType == "OBJECT_FINALIZE"
                )
            );
    }
...
public class BlobOverASocketTests
{
    ...
    )
    {
        ...
        sink.Received()
            .Arrived(
                Arg.Is<Upload>(upload =>
                    upload.Name == "socket.pdf" && upload.Size == 2048 && upload.Bucket == "uploads"
                )
            );
    }
    ...
    {
        sink.When(one => one.Arrived(Arg.Any<Upload>()))
            .Do(_ => throw new InvalidOperationException("refused"));

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            blobs.Uploads(new Upload { Name = "a.txt" })
        );
    }
}
...
public class PipelineBlobTests
{
    ...
    )
    {
        ...
        sink.Received()
            .Arrived(Arg.Is<Upload>(upload => upload.Name == "pipeline.pdf" && upload.Size == 7));
    }
}
