...
public class DirectInvokeTests
{
    ...
    {
        var receipt = await invocations.Handle(
            new OrderRequest
            {
                Id = "o-1",
                Records = ["a", "b"],
                RequestContext = "from-billing",
            }
        );
        ...
        log.Received()
            .Placed(Arg.Is<OrderRequest>(one => one.Id == "o-1" && one.Records.Count == 2));
    }
    ...
    {
        log.When(one => one.Placed(Arg.Any<OrderRequest>()))
            .Do(_ => throw new InvalidOperationException("refused"));

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            invocations.Handle(new OrderRequest { Id = "o-2" })
        );
    }
}
...
public class DirectInvokeOverASocketTests
{
    ...
    )
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            invocations.Handle(new OrderRequest { Id = "s-2" })
        );
    }
}
