...
public class EventarcEnvelopeTests
{
    ...
    public void ABinaryEventRoutesOnSourceAndTypeAndBindsItsData()
    {
        var delivery = Deliveries.CloudEvent(
            ...
            "com.acme.orders",
            "orders/o-1",
            more: ("ce-traceparent", "00-abc-def-01")
        );
    ...
    public void AStructuredEventReadsTheSame()
    {
        var request = Deliveries.Unwrap(
            Envelope,
            Deliveries.Request("POST", "/", "application/cloudevents+json"),
            """
    ...
    public void ASourceWithSlashesIsTheRouteAsWritten()
    {
        var delivery = Deliveries.CloudEvent(
            ...
            "//cloudaudit.googleapis.com/projects/p/logs/activity"
        );

        var request = Deliveries.Unwrap(Envelope, delivery, "{}")!;

        Assert.Equal(
            "///cloudaudit.googleapis.com/projects/p/logs/activity/google.cloud.audit.log.v1.written",
            request.Path
        );
    }
    ...
    public void ARequestThatClaimsToBeACloudEventAndIsNotIsRefused()
    {
        var failure = Assert.Throws<InvalidOperationException>(() =>
            Deliveries.Unwrap(
                Envelope,
                Deliveries.Request("POST", "/", "application/json", ("ce-specversion", "1.0")),
                "{}"
            )
        );
    ...
    public void ACloudEventInEitherFormIsRecognised(
        string method,
        string contentType,
        bool binaryHeaders,
        bool recognised
