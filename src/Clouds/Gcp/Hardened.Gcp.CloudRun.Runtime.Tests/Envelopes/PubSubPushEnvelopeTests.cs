...
public class PubSubPushEnvelopeTests
{
    ...
    private static TestExecutionRequest Post(
    ...
    public void TheMetadataGetsPrefixedHeaders()
    {
        ...
        Assert.Equal(
            "2070443601311540",
            request.Headers[PubSubPushEnvelope.MessageIdHeader].ToString()
        );
        Assert.Equal(
            "2021-02-26T19:13:55.749Z",
            request.Headers[PubSubPushEnvelope.PublishTimeHeader].ToString()
        );
        Assert.Equal(
            "customer-7",
            request.Headers[PubSubPushEnvelope.OrderingKeyHeader].ToString()
        );
        Assert.Equal(
            "projects/myproject/subscriptions/orders",
            request.Headers[PubSubPushEnvelope.SubscriptionHeader].ToString()
        );
    ...
    {
        var request = Unwrap(
            """{"message":{"messageId":"1"},"subscription":"projects/p/subscriptions/orders"}"""
        )!;
    ...
    public void DataThatIsNotBase64IsRefused()
    {
        var failure = Assert.Throws<InvalidOperationException>(() =>
            Unwrap(
                """{"message":{"data":"not base64!"},"subscription":"projects/p/subscriptions/orders"}"""
            )
        );
