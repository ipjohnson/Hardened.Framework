...
public class StorageEnvelopeTests
{
    ...
    public void AnEventarcObjectEventCarriesTheNotificationAttributesAsHeaders()
    {
        ...
        Assert.Equal(
            "1588778055917163",
            request.Headers[StorageEnvelope.GenerationHeader].ToString()
        );
        Assert.Equal(
            StorageEnvelope.ObjectTypePrefix + "deleted",
    ...
    public void ASizeWrittenAsANumberReadsTheSame()
    {
        ...
        var request = Deliveries.Unwrap(
            Envelope,
            delivery,
            """{"bucket":"uploads","name":"a.txt","size":12,"generation":7}"""
        )!;
    ...
    public void ANotificationWithNoPayloadBindsWhatTheAttributesSay()
    {
        ...
        Assert.Equal(
            ...
            body.TryGetProperty("size", out var size) ? size.ValueKind : JsonValueKind.Undefined
        );
    }
    ...
    public void ACloudEventOfAnotherTypeIsDeclined()
    {
        Assert.False(
            Envelope.Recognises(
                Deliveries.CloudEvent("google.cloud.pubsub.topic.v1.messagePublished", "/s")
            )
        );
        Assert.Null(
            Deliveries.Unwrap(
