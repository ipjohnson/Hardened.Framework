...
Hardened serializes JSON with `System.Text.Json`. A source-generated `JsonSerializerContext`
registered as an `IJsonTypeInfoResolver` is read by every JSON serializer in the pipeline: the
request body, the response body, and each item of a streamed response.
...
```json
{ "id": 1, "title": "Read the generated code", "priority": "inProgress" }
```

The property names are camelCase from `JsonSerializerDefaults.Web`. The enum went on the wire as
`inProgress` with no converter written, because the build writes one per enum.
...
worth registering. The project templates ship this wiring.

## Enum vocabulary

`[JsonEnumNaming]` decides an enum's wire values, and it governs the published document as well
as the wire. The build writes a converter per enum, registers it for the JSON body and for the
parameter binder, and writes the same values into the `enum` array of the generated OpenAPI
description. The default is camelCase. To choose something else, set it for the assembly, for one
enum, or both, and the enum wins:
...
Three things are left alone. A `[Flags]` enum, because a flags value is a combination of members
rather than one of them. An enum from a referenced library, which opts in by that library
declaring `[assembly: JsonEnumNaming]` of its own. And aliased members, where two members sharing
a value keep the first declared, which is the one `Enum.ToString` picks.
...
|---|---|---|
| `[JsonEnumNaming]` | Body, parameters and the document | Any enum |
...
`JsonSerializerDefaults.Web`, camelCase, case-insensitive matching and `AllowReadingFromString`
included, unless you rebuild all of it. Set both `SerializeOptions` and `DeSerializerOptions` or
the application writes what it will not read.
...
`JsonStringEnumConverter` to the options therefore overrides converters generated from an OpenAPI
or Smithy contract, and writes the C# member name where the document declares something else:
`"ScienceFiction"` for a schema whose value is `science-fiction`, and a 400 reading it back.
Attach converters to types, or scope them to a context.
:::
...
A published AOT application has no reflection fallback. Every type on the wire must be declared
in a registered context, and one that is missing throws a `NotSupportedException` naming it.
...
supported route. If you reach for it anyway, the non-generic form builds a converter per enum at
run time, which AOT cannot do:
...
`[JsonSourceGenerationOptions]`. It is a warning, so `TreatWarningsAsErrors` is what makes it stop
a build. An enum reachable from no context gets no diagnostic at all. It works locally and fails
after publishing.

`UseStringEnumConverter` is AOT-safe but writes the C# member name and does not reach the
...
- [The OpenAPI document](/guide/openapi-document): where the enum vocabulary is published
- [Streaming responses](/guide/streaming): one JSON document per item
