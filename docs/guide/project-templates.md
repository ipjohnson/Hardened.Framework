...
One command installs the templates, one more writes a project that builds, tests and serves.
...
to every route, because the generator that writes the routing table was never there. Nothing fails;
the application is just empty. The templates reference the right generators, pin every version in
one place, and lay the projects out so the host can be swapped without touching application code.
:::
...
`--response-model` decides how the generated handlers declare what they can answer with. The sample
is written to show the difference: the same three routes answer 404 and 409 in every mode, while
creating a todo answers 200 under `standard` and 201 under the other two. See
[Declared responses](/guide/responses).

`union` generates a `net11.0` project pinned to the .NET 11 SDK in `global.json`. It cannot be
...
`kestrel` is the default. It serves HTTP through Kestrel without the ASP.NET Core request pipeline.
...
interface, the routes and the validation its constraints describe are all generated from it.

`smithy` — the same generated output from a [Smithy model](/guide/smithy). Needs the
[Smithy CLI][smithy] on `PATH` at the pinned version; the build names the version it expects if
yours differs.
...
There is no host project and no `Program.cs`. The deployed artifact is the assembly and the
generator writes the entry point, so adding a `Main` gives the assembly a second entry point and the
build fails.
...
AWS account and nothing to deploy.
...
There is no `AddAcmeGreeting()` to call and no options object to thread through.
...
the runtime it targets ship together, so they move as a set. `--hardened-version` overrides it.
...
two repositories release in sequence, so for a short window the framework is ahead and an exact pin
would name a version that does not exist yet. The float resolves to the newest published
Hardened.Amz.
:::

::: warning Everything is prerelease
`dotnet new install Hardened.Templates` picks the newest prerelease because there is no stable one
to prefer, but adding a package by hand needs
...
binding and the module registration, as ordinary C#.
