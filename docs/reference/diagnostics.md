...
| `006` | Warning | The reader reports a problem in a description it could read. For OpenAPI, among others, scopes on a scheme that carries none. For Smithy, among others, a member narrowed to `decimal` or `long`, a trait it does not model, a `@timeout` of zero or less, or, in a committed AST, an `@validation` that is not `stop-on-first-error` or `collect-all` | Build task | `HOAT006`: [Generating from OpenAPI](/guide/openapi). `HSMT006`: [Generating from Smithy](/guide/smithy) |
