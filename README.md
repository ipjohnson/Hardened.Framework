...
A generated client is a parameter too. `[assembly: KiotaTesting]`, from `Hardened.Kiota.Testing`,
builds it over an `HttpClient` whose handler is the pipeline, so the client's calls, its models and
its typed exceptions all run against the real application with no socket, and `Returns<T>()`
asserts a call by naming the response type the contract declares - the status, the body type and
the headers that status carries, in one word:
